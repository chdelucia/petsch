...
describe('ObservabilityFacade', () => {
  ...
  it('should call analytics.sendEvent when trackEvent is called', () => {
    ...
    expect(analyticsMock.sendEvent).toHaveBeenCalledWith('test', {
      key: 'val',
    });
  });
  ...
  it('should call analytics.trackCart when trackAddToCart is called', () => {
    facade.trackAddToCart('1', 'title', 100);
    expect(analyticsMock.trackCart).toHaveBeenCalledWith('1', 'title', 100);
  });

  it('should call trackEvent with purchase when trackPurchase is called', () => {
    facade.trackPurchase(500);
    expect(analyticsMock.sendEvent).toHaveBeenCalledWith('purchase', {
      total: 500,
    });
  });
});
