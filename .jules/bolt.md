...
## 2026-05-15 - [Component Optimization with OnPush and Signals]
**Learning:** In a zoneless Angular application, implementing `ChangeDetectionStrategy.OnPush` is essential for minimizing change detection cycles. Components that use Signals for state (like `isMenuOpen` and `currentApp`) are perfect candidates for `OnPush`. Additionally, wrapping static arrays in `computed` signals, while having minor overhead, aligns with a consistent reactive architecture and benefits from memoization if the source inputs were to become reactive in the future.
**Action:** Default to `OnPush` for all new components and refactor existing ones that use Signals to use `OnPush`.
